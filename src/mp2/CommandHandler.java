...
public class CommandHandler {
    ...
    private void handleFileSystemCommands(String command) {
        ...
        switch (commandList[0]) {
            ...
            case Command.STORAGE:
                handleStorage(commandList);
                break;
            case Command.DIFF:
                handleDiff(commandList);
                break;
            case Command.MORE:
                handleMore(commandList);
                break;
            case Command.CLEAR:
                handleClear(commandList);
                break;
            case Command.COMPARE:
                handleCompare(commandList);
                break;
            default:
                logger.warning("Invalid command");
                break;
    ...
    private void handleGet(String[] command) {
        if(command.length != 3) {
            ...
            this.sender.sendPreGetRequest(command[1], command[2]);
        }
    }
    ...
    private void handlePut(String[] command) {
        if(command.length != 3) {
            ...
            this.sender.sendPrePutRequest(command[1], command[2]);
        }
    }
