...
import mp3.message.JuiceAckRequest;
...
public class MasterReceiver extends Receiver {
    ...
    // map source file to number of maple exe
    private final Map<String, Integer> mapleRunningServers;
    // map dest file to number of juice exe
    private final Map<String, Integer> juiceRunningServers;
    // map dest file to intermediate prefix at juice stage
    private final Map<String, String> destToIntermediate;
    // map input file name to those servers assigned tasks
    private final Map<String, Set<ServerInfo>> runningServers;
    // map server to its assigned tasks
    private final Map<ServerInfo, Task> assignedTasks;
    ...
    private void receive(String msg) {
        ...
        switch (msgType) {
            case (MsgType.MAPLE_REQUEST) -> handleMapleRequest(msgJson);
            case (MsgType.MAPLE_FILE_MSG) -> handleMapleFileMsg(msgJson);
            case (MsgType.MAPLE_COMPLETE_MSG) -> handleMapleCompleteMsg(msgJson);
            case (MsgType.MAPLE_ACK_REQUEST) -> handleMapleAckRequest(msgJson);
            case (MsgType.MAPLE_ACK) -> handleMapleAck(msgJson);
            case (MsgType.JUICE_REQUEST) -> handleJuiceRequest(msgJson);
            case (MsgType.JUICE_FILES_MSG) -> handleJuiceFilesMsg(msgJson);
            case (MsgType.JUICE_COMPLETE_MSG) -> handleJuiceCompleteMsg(msgJson);
            case (MsgType.JUICE_ACK_REQUEST) -> handleJuiceAckRequest(msgJson);
            case (MsgType.JUICE_ACK) -> handleJuiceAck(msgJson);
            case (MsgType.JOIN_REQUEST) -> handleJoinRequest(msgJson);
            case (MsgType.SERVER_FAIL) -> handleFailure(msgJson);
            case (MsgType.FP_REJOIN_MSG) -> handleFPRejoinMsg(msgJson);
        }
    }
    ...
    private void handleMapleRequest(JSONObject msgJson) {
        ...
        List<String> splitFiles = FileSplitter.split(
            sourceName,
            mapleNum
        );
        ...
        if (splitFiles != null) {
            for (int i = 0; i < splitFiles.size(); i++) {
                ...
                Message mapleFileMsg = new MapleFileMsg(
                    sourceName,
                    splitFileName,
                    intermediatePrefix,
                    mapleExe
    ...
    private void handleMapleCompleteMsg(JSONObject msgJson) {
        ...
        if (mapleServers == null || !(mapleServers.contains(new ServerInfo(
            senderIpAddress,
            senderPort
        )))) {
        ...
        String mapleOutputPath = getMapleOutputPath(
            sourceFileName,
            intermediatePrefix,
            destFileName
        );
        File dir = new File(getMapleOutputDir(
            sourceFileName,
            intermediatePrefix
        ));
        dir.mkdir();
        this.dataTransfer.receiveFile(
            mapleOutputPath,
            FilePath.INTERMEDIATE_PATH + destFileName,
            senderIpAddress
        );
        Message requestAck = new MapleAckRequest(
            sourceFileName,
            intermediatePrefix
        );
    ...
    private void handleMapleAck(JSONObject msgJson) {
        ...
        mapleRunningServers.put(
            sourceFile,
            mapleRunningServers.get(sourceFile) - 1
        );
    ...
    private void sortPairsByKeys(
        String sourceFile,
        String intermediatePrefix
    ) {
        String directoryPath = getMapleOutputDir(
            sourceFile,
            intermediatePrefix
        );
    ...
    private void handleJuiceRequest(JSONObject msgJson) {
        ...
        this.destToIntermediate.put(
            destFileName,
            intermediatePrefix
        );
        File[] files = dir.listFiles();
        if (files != null) {
            int juiceNum = Math.min(
                files.length,
                msgJson.getInt(MsgKey.NUM_JUICE)
            );
            juiceNum = Math.min(
                this.servers.size(),
                juiceNum
            );
            this.juiceRunningServers.put(
                destFileName,
                juiceNum
            );
    ...
    private void handleJuiceCompleteMsg(JSONObject msgJson) {
        ...
        if (juiceServers == null || !(juiceServers.contains(new ServerInfo(
            senderIpAddress,
            senderPort
        )))) {
            // false positive occurs
        ...
        String juiceOutputTmpPath = getJuiceOutputTmpPath(
            destFile,
            senderIpAddress,
            senderPort
        );
        ...
        try {
            ...
            fOut = new BufferedWriter(new FileWriter(
                juiceOutputRemotePath,
                true
            ));
            String line = null;
            while ((line = fIn.readLine()) != null) {
        ...
        int isDelete = msgJson.getInt(MsgKey.IS_DELETE);
        Message juiceRequestAck = new JuiceAckRequest(
            destFile,
            isDelete
        );
        this.dataTransfer.send(
            juiceRequestAck.toJSON(),
            senderIpAddress,
            senderPort
        );
    }
    ...
    private void handleJuiceAck(JSONObject msgJson) {
        ...
        juiceRunningServers.put(
            destFile,
            juiceRunningServers.get(destFile) - 1
        );
    ...
    private void handleFailure(JSONObject msgJson) {
        ...
        ServerInfo failedServer = new ServerInfo(
            failedIpAddress,
            failedPort
        );
    ...
    private String getMapleOutputPath(
